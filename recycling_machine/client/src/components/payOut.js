...
const addressRegex = /T[A-Za-z1-9]{33}/;
const States = {
  BEGIN: 'begin',
...
const PayOut = () => {
  ...
  const calculateTokenBalance = async () => {
    ...
    if (paymentOption === 'trx') {
      const price = await axios.get("/api/getTrxUsdPrice");
      console.log(price.data);
      setTokenBalance((currentBalance / price.data / 100).toFixed(6));
    }
    else {
      setTokenBalance(currentBalance / 100);
    }
  }
